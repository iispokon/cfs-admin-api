...
class OperationNotAllowed(CloudFSError):
    def __init__(self, op_name):
        self.message = '{} is not possible in Cloudfs. If you cannot work around this limitation, please contact CloudFS support.'.format(
            op_name)
...
class MethodNotImplemented(CloudFSError):
    def __init__(self, object, method_name):
        self.message = 'The \"{}\" method of the {} is not currently implemented. To find out the future plans for this method contact CloudFS support.'.format(
            method_name, type(object))
...
class AuthenticatedError(CloudFSError):
    ...
    def __init__(self, request, response, message='', code=None):
        ...
        if code and type(self.INTERNAL_CODE) is tuple:
            if code in self.INTERNAL_CODE:
                ...
                raise ValueError(
                    "Tried to create a {} using error code {}, which is not in the range of {}!".format(type(self),
                                                                                                        code,
                                                                                                        self.INTERNAL_CODE))
    ...
    def __str__(self):
        return '\nRequest:\n{}\nResponse:\n{}'.format(request_to_string(self.request),
                                                      response_to_string(self.response))
...
def error_from_response(request, response):
    ...
    if 'error' in response_json:
        if response_json['error'] == 'invalid_request':
            ...
            errcode = int(response_json['error']['code'])
            message = response_json['error']['message']
            try:
                error_class = _error_index[errcode]
            except KeyError:
            ...
            print "returning error_class request={} response={} message={}".format(request, response, message)
